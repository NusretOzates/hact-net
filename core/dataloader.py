...
class BRACSDataset(Dataset):
    ...
    def _load_assign_mat(self):
        ...
        if self.load_in_ram:
            self.assign_matrices = [h5_to_tensor(fname).float().t() for fname in self.assign_fnames]
    ...
    def __getitem__(self, index):
        ...
        if hasattr(self, "num_tg") and hasattr(self, "num_cg"):
            if self.load_in_ram:
                ...
                assert self.cell_graph_labels[index] == self.tissue_graph_labels[index], (
                    "The CG and TG are not the same. There was an issue while creating HACT."
                )
...
def collate(batch):
    ...
    batch = tuple([collate_fn(batch, mod_id, type(batch[0][mod_id]).__name__) for mod_id in range(num_modalities)])

    return batch


def make_data_loader(batch_size, shuffle=True, num_workers=0, **kwargs):
    """
    Create a BRACS data loader.
    """
    dataset = BRACSDataset(**kwargs)
    dataloader = torch.utils.data.DataLoader(
        dataset, batch_size=batch_size, shuffle=shuffle, num_workers=num_workers, collate_fn=collate
    )
